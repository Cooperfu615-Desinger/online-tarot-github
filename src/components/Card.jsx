...
const Card = ({ data, positionName, isRevealed, onReveal, index, isNew }) => {
    return (
        <div className={`flex flex-col items-center group perspective-1000 flex-shrink-0 ${isNew ? 'animate-fly-in-from-right' : ''}`}>
            ...
            >
                {/* 卡牌背面 */}
                <div className="absolute inset-0 w-full h-full bg-gradient-to-br from-indigo-900 to-purple-950 border-2 border-amber-600/50 rounded-lg flex items-center justify-center backface-hidden shadow-xl overflow-hidden">
                ...
                {/* 卡牌正面 */}
                <div className="absolute inset-0 w-full h-full bg-slate-100 border-4 border-amber-500 rounded-lg flex flex-col items-center justify-between p-2 backface-hidden rotate-y-180 shadow-[0_0_15px_rgba(251,191,36,0.4)] overflow-hidden">
                    ...
                    <div className="text-center">
                        ...
                        <h3 className="text-sm font-bold text-slate-900 leading-tight">{data.name}</h3>
                    </div>
